...
//
// Last Modified: Thu Nov  9 12:43:28 CET 2023
...
extern "C" double pow(double, double)
{
    ...
    fprintf(stderr, "DO NOT USE POW! DON\'T WRITE YOUR OWN POW EITHER!\nPOWER OF TWO: 1 << n\n");
    abort();
}
// }}}
...
void test_insert_it_stream(ads::set<val_t>& a, std::set<val_t>& r) {
    std::cerr << "\n=== test_insert_it ===\n";

    std::cerr << "insert stream 1 3 5 7 9 11 13\n";
    std::vector<val_t> values{ 1, 3, 5, 7, 9, 11, 13 };

    std::istringstream strrr("1 3 5 7 9 11 13");
    auto first = std::istream_iterator<val_t>(strrr);
    auto last  = std::istream_iterator<val_t>();

    a.insert(first, last);
    r.insert(values.begin(), values.end());

    sanity_check("insert_it_stream", a, r);
}
...
void test_assign(ads::set<val_t> const& a, std::set<val_t> const& r) {
    ...
    ads::set<val_t> c{ 1, 3, 666 };
    c = a;

    for(auto const* p: std::array<ads::set<val_t> const*, 2>{&b, &c}) {
        if(a.size() != p->size()) {
            std::cerr << RED("[copy] err: size after assigning does not match.\n"
                      << "size should be: " << a.size() << ", but is " << p->size() << '\n');
